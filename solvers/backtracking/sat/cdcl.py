...
def parse_dimacs_cnf(path_to_file: str) -> Formula:
...
def main():
    parser = argparse.ArgumentParser(description='An CDCL-SAT solver')
    ...
    sat_problem = parse_dimacs_cnf(args.file_name[0])

    result = cdcl_solve(sat_problem)
    if result:
        assert result.satisfy(sat_problem)
        print('True')
